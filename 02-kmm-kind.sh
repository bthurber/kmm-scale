...
# Change to the kind-hub kubectl/oc context first and then run:
make deploy-hub
...
### Applying a dummy ManagedClusterModule CR on the hub
...
spec:
  ...
  selector:
    cluster.open-cluster-management.io/clusterset: test
  moduleSpec:
    imageRepoSecret:
      name: mresvani-quay-cred
    moduleLoader:
      container:
        modprobe:
